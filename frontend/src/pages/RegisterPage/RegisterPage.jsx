 ...
 * Валидация формы на клиенте + на бэкенде (Zod).
 */
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Не удалось зарегистрироваться. Попробуйте позже.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="register-page__form" noValidate>
        ...
        <div className="form-group">
          ...
          <input id="name" name="name" type="text" value={formData.name}
            onChange={handleChange} placeholder="Ваше имя" required autoComplete="name" />
        </div>

        <div className="form-group">
          <label htmlFor="email">Email</label>
          <input id="email" name="email" type="email" value={formData.email}
            onChange={handleChange} placeholder="example@mail.com" required autoComplete="email" />
        </div>

        <div className="form-group">
          <label htmlFor="password">Пароль</label>
          <input id="password" name="password" type="password" value={formData.password}
            onChange={handleChange} placeholder="Минимум 8 символов" required minLength={8}
        ...
        <button type="submit" className="btn btn--primary register-page__submit" disabled={loading}>
